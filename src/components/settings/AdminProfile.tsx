...
export function AdminProfile({ value, onEdit }: AdminProfileProps) {
  const displayName = value.name.trim() || 'Administrator'
  const displayEmail = value.email.trim() || 'Not configured'
  const displayRole = value.role.trim() || 'Administrator'
  const displayLibrary = value.library.trim() || 'LibSync Central Library'
