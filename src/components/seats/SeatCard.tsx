...
export function SeatCard({ seat, onClick }: SeatCardProps) {
  const Icon = seat.status === 'free' ? Armchair : seat.status === 'occupied' ? UserRound : Construction
  const label = seat.status === 'free' ? 'Free' : seat.status === 'occupied' ? 'Occupied' : 'Maintenance'
