...
export function SettingsPage() {
  ...
  const [adminProfileOverride, setAdminProfileOverride] = useState<MockSettings['admin'] | null>(null)
  const [counts, setCounts] = useState({ total: 0, available: 0, occupied: 0, maintenance: 0 })
  ...
  useEffect(() => {
    ...
    fetchSeats()
      .then((seats) => {
        if (isMounted) {
          setCounts(getSeatCounts(seats))
        }
      })
      .catch((err) => {
        console.warn('[SettingsPage] Notice: Could not load real seat counts:', err)
      })
    return () => {
      isMounted = false
    }
  ...
  const resetSettings = () => {
    ...
    setAdminProfileOverride(null)
    setErrors({})
    setSuccessMessage('Settings restored to system defaults.')
    setShowReset(false)
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">Settings</h2>
          <p className="mt-1 text-xs text-slate-500 sm:text-sm">Manage library configuration, preferences, and administrator settings.</p>
        </div>
        <div className="flex gap-2">
          <Button type="button" variant="outline" size="md" icon={<RotateCcw className="h-4 w-4" />} onClick={() => setShowReset(true)}>Reset</Button>
          <Button type="button" variant="primary" size="md" icon={<Save className="h-4 w-4" />} onClick={saveChanges}>Save Changes</Button>
        </div>
      </div>
      {successMessage && (
        <div className="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 p-3.5 text-sm text-emerald-800">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[220px_1fr]">
        ...
        <div className="min-w-0">
          {activeSection === 'library' && (
            <Card>
              <CardHeader title="Library Information" subtitle="Basic identity and operating hours" />
              <CardContent><LibraryInformation value={draft.library} errors={errors} onChange={updateLibrary} /></CardContent>
            </Card>
          )}
          {activeSection === 'seats' && (
            <Card>
              <CardHeader title="Seat Configuration" subtitle="Capacity and section preferences for library seats" />
              <CardContent><SeatConfiguration value={draft.library} counts={counts} onChange={updateLibrary} /></CardContent>
            </Card>
          )}
          {activeSection === 'preferences' && (
            <Card>
              <CardHeader title="Library Preferences" subtitle="Display and workflow options" />
              <CardContent><LibraryPreferences value={draft.preferences} onChange={updatePreference} /></CardContent>
            </Card>
          )}
          {activeSection === 'administrator' && (
            <Card>
              <CardHeader title="Administrator" subtitle="Authenticated administrator account details" />
              <CardContent><AdminProfile value={currentAdmin} onEdit={() => setShowProfile(true)} /></CardContent>
            </Card>
          )}
          <div className="mt-4 flex items-center gap-2 text-xs text-slate-400">
            <Settings2 className="h-4 w-4" />Settings preferences are preserved in your current session.
          </div>
        </div>
      </div>
...
function ResetConfirmation({ onCancel, onConfirm }: { onCancel: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button type="button" className="absolute inset-0 bg-slate-900/40 backdrop-blur-2xs" onClick={onCancel} aria-label="Close reset confirmation" />
      <div role="dialog" aria-modal="true" aria-labelledby="reset-title" className="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
        <h2 id="reset-title" className="font-semibold text-slate-900">Reset settings?</h2>
        <p className="mt-1 text-sm leading-relaxed text-slate-500">Restore all library information, preferences, and section options to system defaults.</p>
        <div className="mt-6 flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={onCancel}>Cancel</Button>
          <Button type="button" variant="danger" onClick={onConfirm}>Reset settings</Button>
        </div>
      </div>
    </div>
  )
